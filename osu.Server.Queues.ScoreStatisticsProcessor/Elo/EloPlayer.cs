...
namespace osu.Server.Queues.ScoreStatisticsProcessor.Elo
{
    [Serializable]
    public class EloPlayer
    {
        ...
        public Queue<EloTanhTerm> LogisticFactors = [];

        public EloRating LastPerformance = new EloRating();
        ...
        public void AddNoiseBest(double sigNoise, double transferSpeed)
        {
            ...
            foreach (var factor in LogisticFactors)
        ...
        public void UpdatePerformance(DateTimeOffset contestTime, EloRating performance, int maxHistory)
        {
            ...
            {
                EloTanhTerm logisticFactor = LogisticFactors.Dequeue();

                double wN = Math.Pow(NormalFactor.Sig, -2);
                double wL = logisticFactor.GetWeight();

                NormalFactor = new EloRating
                (
                    (wN * NormalFactor.Mu + wL * logisticFactor.Mu) / (wN + wL),
                    Math.Pow(wN + wL, -0.5)
                );
            }

            LogisticFactors.Enqueue(new EloTanhTerm(performance));
